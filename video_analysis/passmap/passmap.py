...
class PassMap:
    ...
    def player_average_coord(self):
        ...
        for player_name in pass_player_name:
            coord_data = self.df_players_with_ball[
                self.df_players_with_ball["player_name"] == player_name
            ]["coord_tr"].tolist()
            coord_average = [(sum(item) / len(item)) for item in zip(*coord_data)]
            passmap_bbox.append(
                {
                    "player_name": player_name,
                    "team": self.df_players_with_ball[
                        self.df_players_with_ball["player_name"] == player_name
                    ]["team"].tolist()[0],
    ...
    def create_passmap_data(self, match_id: str, pass_df_path: str):
        ...
        for i in range(len(players_with_ball) - 1):
            if players_with_ball[i]["team"] == players_with_ball[i + 1]["team"]:
                if (
                    players_with_ball[i]["player_name"]
                    != players_with_ball[i + 1]["player_name"]
                ):
                ...
                passmap_fail.append(
                    {
                        ...
                        "interceptor_initial": players_with_ball[i + 1][
                            "player_initial"
                        ],
        ...
        for i in passmap_success:
            i["passer_x"] = self.df_passmap_bbox[
                self.df_passmap_bbox["player_name"] == i["passer_name"]
            ]["coord_average_x"].item()
            i["passer_y"] = self.df_passmap_bbox[
                self.df_passmap_bbox["player_name"] == i["passer_name"]
            ]["coord_average_y"].item()
            i["receiver_x"] = self.df_passmap_bbox[
                self.df_passmap_bbox["player_name"] == i["receiver_name"]
            ]["coord_average_x"].item()
            i["receiver_y"] = self.df_passmap_bbox[
                self.df_passmap_bbox["player_name"] == i["receiver_name"]
            ]["coord_average_y"].item()
        ...
        for i in passmap_fail:
            ...
            i["interceptor_x"] = self.df_passmap_bbox[
                self.df_passmap_bbox["player_name"] == i["interceptor_name"]
            ]["coord_average_x"].item()
            i["interceptor_y"] = self.df_passmap_bbox[
                self.df_passmap_bbox["player_name"] == i["interceptor_name"]
            ]["coord_average_y"].item()
    ...
    def passmap_plot(self, passmap_viz_path: str, match_id: str):
        ...
        df_passmap_split = {
            "Team-A": df_passmap.query("start_pitch_side=='left'"),
        ...
        for team, df_passmap in df_passmap_split.items():
            ...
            avg_positions = df_passmap.groupby("passer_name")[
                ["passer_x", "passer_y"]
            ].mean()
...
#
#     def create_passmap_data(self, match_id: str, pass_df_path: str):
#         pass_df_path = os.path.join(self.project_dir, pass_df_path)
#         if not os.path.exists(pass_df_path):
#             print("dir for pass df not exists\nmaking dir...")
#             os.makedirs(pass_df_path)
#             print("dir for pass df created!")
#         else:
#             print("dir for pass df exists!")
#             if len(glob(os.path.join(pass_df_path, f"{match_id}*"))) == 2:
#                 print("pass df already exists!")
#                 self.df_passmap_success = pd.read_csv(
#                     os.path.join(pass_df_path, f"{match_id}-pass-success-df.csv")
#                 )
#                 self.df_passmap_fail = pd.read_csv(
#                     os.path.join(pass_df_path, f"{match_id}-pass-fail-df.csv")
#                 )
#         passmap_success = []
#         passmap_fail = []
#         players_with_ball = self.df_players_with_ball.to_dict("records")
...
#                         "frame": players_with_ball[i]["frame"],
#                         "passer_name": players_with_ball[i]["player_name"],
#                         "passer_initial": players_with_ball[i]["passer_initial"],
#                         "passer_coord": players_with_ball[i]["coord_tr"],
#                         "interceptor_name": players_with_ball[i + 1]["player_name"],
#                         "interceptor_initial": players_with_ball[i + 1][
#                             "player_initial"
...
#
#         self.df_passmap_success = pd.DataFrame(passmap_success)
#         self.df_passmap_success.to_csv(
#             os.path.join(pass_df_path, f"{match_id}-pass-success-df.csv")
#         )
#         self.df_passmap_fail = pd.DataFrame(passmap_fail)
#         self.df_passmap_fail.to_csv(
#             os.path.join(pass_df_path, f"{match_id}-pass-fail-df.csv")
#         )
#
#     def passmap_plot(self, passmap_viz_path: str, match_id: str):
#         passmap_viz_path = os.path.join(self.project_dir, passmap_viz_path)
#         if not os.path.exists(passmap_viz_path):
#             print("no dir for passmap viz\nmaking dir...")
#             os.makedirs(passmap_viz_path)
#             print("dir for passmap viz created!")
#         else:
#             print("dir exists")
#             if len(glob(os.path.join(passmap_viz_path, f"{match_id}*"))) == 2:
#                 print("pass map viz already exists!")
#                 return glob(os.path.join(passmap_viz_path, f"{match_id}*"))
#         print("making passmap vizs...")
#         plt.rcParams["axes.unicode_minus"] = False
#         f_path = os.path.join(self.project_dir, "font/malgun.ttf")
#         font_name = font_manager.FontProperties(fname=f_path).get_name()
#         rc("font", family=font_name)
#         df_passmap = self.df_passmap_success
#         df_passmap_split = {
...
#                     passmap_viz_path, f"{match_id}-passmap-team_a.png"
#                 )
#             else:
#                 output_path = os.path.join(
#                     passmap_viz_path, f"{match_id}-passmap-team_b.png"
#                 )
#             plt.savefig(output_path, dpi=300, bbox_inches="tight")
#             print("passmap viz created!")
#         return glob(os.path.join(passmap_viz_path, f"{match_id}*"))
